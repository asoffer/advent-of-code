...
fn step(prev: &HashSet<(i32, i32, i32)>) -> HashSet<(i32, i32, i32)> {
    let mut neighbors = HashMap::<(i32, i32, i32), usize>::new();
    for (x, y, z) in prev.iter() {
        for xx in (x-1)..=(x+1) {
            for yy in (y-1)..=(y+1) {
                for zz in (z-1)..=(z+1) {
                    if xx == *x && yy == *y && zz == *z { continue; }
                    *neighbors.entry((xx, yy, zz)).or_insert(0) += 1;
                }
            }
        }
    }
...
fn main() {
    let mut active_cells = HashSet::<(i32, i32, i32)>::new();
    ...
    for (x, line) in contents.lines().enumerate() {
        for (y, c) in line.chars().enumerate() {
            if c == '#' {
                active_cells.insert((x as i32, y as i32, 0));
            }
        }
    }
    for _ in 0..6 {
